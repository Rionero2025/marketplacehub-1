...
from services.accounting import (
    ...
    refresh_accounting_costs,
    save_export,
    save_accounting_inline_edits,
    save_accounting_catalog_selection,
    save_manual_fields,
...
if review_active:
    ...
    review_records = accounting_rows(seller_id, account_id, marketplace)
...
if catalog_options:
    ...
    if catalog_all_col.button(
        ...
        save_accounting_catalog_selection(seller_id, catalog_ids)
        st.rerun()
    if catalog_none_col.button(
        ...
        st.session_state[catalog_choice_key] = []
        save_accounting_catalog_selection(seller_id, [])
        st.rerun()
    ...
    if set(selected_catalog_ids) != set(current_enabled_ids) or not catalog_selection["configured"]:
        ...
        if set(selected_catalog_ids) != set(current_enabled_ids):
            save_accounting_catalog_selection(seller_id, selected_catalog_ids)
            catalog_selection = accounting_catalog_selection(seller_id)
...
if costs_col.button(
    ...
    try:
        with st.spinner("Ricerca EAN esatto nei listini selezionati…"):
            cost_result = refresh_accounting_costs(
                seller_id,
                account_id,
                marketplace,
                date_from=date_from,
